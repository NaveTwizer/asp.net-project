...
{
    public partial class products_per_provider : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                string query = "SELECT * FROM ProductsPerProvider";
                DataSet ds = GetDataSet(query);
                string table = "<table><tr><td>שם ספק</td><td>סך מוצרים</td></tr>";
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    table += "<tr>";
                    table += "<td>" + row["ProviderName"].ToString() + "</td>";
