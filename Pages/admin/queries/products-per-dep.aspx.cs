...
{
    public partial class products_per_dep : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                string query = "SELECT * FROM ProductsPerDep";
                DataSet ds = GetDataSet(query);
                string table = "<table><tr><td>שם מחלקה</td><td>סך מוצרים</td></tr>";
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    table += "<tr>";
                    table += "<td>" + row["DepName"].ToString() + "</td>";
